<div id="page_content">
    ...
    $inicio = ($pag * $maximo) - $maximo;
        ...
        visitas
        from posts
        where titulo like '%$pesquisa%' or texto like '%$pesquisa%'
        order by id desc
        limit $inicio,$maximo") or die(mysql_error());
        
        if (@mysql_num_rows($noticias) == '0') {
            echo "<h2>Desculpe, sua pesquisa não retornou resultados</h2><br><br>";
        } else {
        ...
        <div class="categoria">
            <a href="index.php?pagina=nav/single&amp;topico=<?php echo $id;?>">
                ...
                <p class="categoria_p"><?php echo strip_tags(trim(limitarTexto($texto, 140)));?>
                </p>
            </a>
        </div>        
        <?php
        ...
        $sql_res = mysql_query("SELECT * FROM posts where titulo like '%$pesquisa%' or texto like '%$pesquisa%'");
        $total = mysql_num_rows($sql_res);

        $paginas = ceil($total/$maximo);
        $links = '5'; //QUANTIDADE DE LINKS NO PAGINATOR
        
        echo "<a href=\"index.php?pagina=nav/search&amp;pesquisa=$pesquisa&amp;pag=1\">Primeira Página</a>&nbsp;&nbsp;&nbsp;";
        ...
        for ($i = $pag-$links; $i <= $pag-1; $i++){
            if ($i <= 0){
                ...
                echo"<a href=\"index.php?pagina=nav/search&amp;pesquisa=$pesquisa&amp;pag=$i\">$i</a>&nbsp;&nbsp;&nbsp;";}
        }echo "$pag &nbsp;&nbsp;&nbsp;";
        ...
        for($i = $pag +1; $i <= $pag+$links; $i++){
            ...
            echo "<a href=\"index.php?pagina=nav/search&amp;pesquisa=$pesquisa&amp;pag=$paginas\">Última página</a>&nbsp;&nbsp;&nbsp;";
        ?>
        </div>
    </div>
</div>
